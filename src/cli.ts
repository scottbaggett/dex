    ...
    .action(async (options) => {
        ...
        if (options.tags) {
            ...
            prompts = prompts.filter(
                (p) => p.tags && p.tags.some((tag) => filterTags.includes(tag)),
            );
        }
        ...
        for (const prompt of prompts) {
            console.log(
                chalk.green.bold(`${prompt.id}`) +
                    chalk.gray(" - ") +
                    chalk.white(prompt.name),
            );
            ...
            if (options.verbose || (prompt.tags && prompt.tags.length > 0)) {
                if (prompt.tags && prompt.tags.length > 0) {
                    console.log(
                        chalk.gray("  Tags: ") +
                            chalk.blue(prompt.tags.join(", ")),
                    );
                }
                if (prompt.extends) {
                    console.log(
                        chalk.gray("  Extends: ") + chalk.cyan(prompt.extends),
                    );
                }
                if (prompt.llm && options.verbose) {
                    console.log(
                        chalk.gray("  Recommended for: ") +
                            chalk.blue(prompt.llm.join(", ")),
                    );
                }
            }
            console.log();
...
promptsCmd
    ...
    .action(async (id, options) => {
        ...
        if (!template) {
            ...
            console.log(
                chalk.gray(
                    'Run "dex prompts list" to see available templates.\n',
                ),
            );
            process.exit(1);
        }
        ...
        if (template.tags && template.tags.length > 0) {
            console.log(
                chalk.blue("Tags:") +
                    " " +
                    chalk.gray(template.tags.join(", ")),
            );
        }

        if (template.extends) {
            console.log(
                chalk.blue("Extends:") + " " + chalk.cyan(template.extends),
            );
        }

        if (template.llm && template.llm.length > 0) {
            console.log(
                chalk.blue("Recommended for:") +
                    " " +
                    chalk.gray(template.llm.join(", ")),
            );
        }
...
promptsCmd
    ...
    .action(async (name, options) => {
        ...
        if (options.output) {
            ...
            console.log(
                chalk.gray(
                    "Save this as a .yml file in your prompts/ directory:\n",
                ),
            );
