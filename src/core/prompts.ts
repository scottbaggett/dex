import type { ExtractedContext, DexOptions } from "../types";
...
export class PromptGenerator {
    ...
    static generate(context: ExtractedContext, options: DexOptions): string {
        ...
        if (options.promptTemplate) {
            ...
            if (template) {
                ...
                if (template.variables) {
                    ...
                    instructions = promptLoader.interpolateVariables(
                        instructions,
                        variables,
                    );
                }
            ...
            console.warn(
                `Prompt template '${options.promptTemplate}' not found, using default`,
            );
        ...
        if (options.interactive && !options.noPrompt) {
            ...
            if (suggestions.length > 0) {
                ...
                console.log(
                    `💡 Suggested prompt template: ${topSuggestion.name}`,
                );
            }
        }
    ...
    private static generateContextualPrompt(
        ...
        if (analysis.task) {
            parts.push(
                `Review these changes in the context of: "${analysis.task}"`,
            );
            parts.push("");
            ...
            const langConcerns = this.getLanguageSpecificConcerns(
                analysis.primaryLanguage,
            );
            if (langConcerns.length > 0) {
                parts.push("");
                parts.push(
                    `${this.getLanguageName(analysis.primaryLanguage)}-specific considerations:`,
                );
    ...
    private static getPromptByName(
        promptName: string,
        analysis: PromptContext,
    ): string {
...
Suggest missing test scenarios.`,
        ...
        return (
            prompts[promptName] ||
            this.generateContextualPrompt(analysis, { format: "markdown" })
        );
    }
}
