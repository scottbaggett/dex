...
export class TypeScriptASTProcessor {
    process(source: string, filePath: string, options: ProcessingOptions): ProcessResult {
        ...
        const visit = (node: ts.Node) => {
            ...
            if (ts.canHaveModifiers(node) && ts.getModifiers(node)?.some(m => m.kind === ts.SyntaxKind.ExportKeyword)) {
                const exportNode = this.extractExport(node, sourceFile, options);
                if (exportNode) {
    ...
    private extractExport(node: ts.Node, sourceFile: ts.SourceFile, options: ProcessingOptions): ExportNode | null {
        ...
        if (ts.isClassDeclaration(node) && node.name) {
            return {
                ...
                line,
                isExported: true,
                members: options.compact ? undefined : this.getClassMembers(node, sourceFile, options)
            };
        }
        ...
        if (ts.isInterfaceDeclaration(node)) {
            return {
                ...
                line,
                isExported: true,
                members: options.compact ? undefined : this.getInterfaceMembers(node, sourceFile, options)
            };
        }
    ...
    private getClassSignature(node: ts.ClassDeclaration, options: ProcessingOptions): string {
        let signature = `class ${node.name?.text || 'Anonymous'}`;
        
        // Add type parameters
        if (node.typeParameters) {
            signature += '<' + node.typeParameters.map(tp => tp.getText()).join(', ') + '>';
        }
        ...
        if (node.heritageClauses) {
            node.heritageClauses.forEach(clause => {
                if (clause.token === ts.SyntaxKind.ExtendsKeyword) {
                    signature += ' extends ' + clause.types.map(t => t.getText()).join(', ');
                } else if (clause.token === ts.SyntaxKind.ImplementsKeyword) {
                    signature += ' implements ' + clause.types.map(t => t.getText()).join(', ');
                }
            });
        }
    ...
    private getInterfaceSignature(node: ts.InterfaceDeclaration, options: ProcessingOptions): string {
    ...
    private getEnumSignature(node: ts.EnumDeclaration, options: ProcessingOptions): string {
        if (options.compact) {
    ...
    private getVariableSignature(node: ts.VariableStatement): string {
        ...
        if (declaration && ts.isIdentifier(declaration.name)) {
            ...
            const initializer = declaration.initializer ? ` = ${declaration.initializer.getText()}` : '';
    ...
    private getClassMembers(node: ts.ClassDeclaration, sourceFile: ts.SourceFile, options: ProcessingOptions): MemberNode[] {
        const members: MemberNode[] = [];
        
        node.members.forEach(member => {
            if (ts.isMethodDeclaration(member) || ts.isConstructorDeclaration(member)) {
                const name = ts.isConstructorDeclaration(member) ? 'constructor' : 
                ...
                if (name && this.shouldIncludeMember(member, options)) {
                    ...
                    const returnType = ts.isMethodDeclaration(member) && member.type ? `: ${member.type.getText()}` : '';
                    const async = member.modifiers?.some(m => m.kind === ts.SyntaxKind.AsyncKeyword) ? 'async ' : '';
    ...
    private getInterfaceMembers(node: ts.InterfaceDeclaration, sourceFile: ts.SourceFile, options: ProcessingOptions): MemberNode[] {
        const members: MemberNode[] = [];
    ...
    private shouldIncludeMember(member: ts.ClassElement, options: ProcessingOptions): boolean {
        ...
        if (isProtected && options.depth === 'public') {
            return false;
        }
    ...
    private shouldInclude(node: ExportNode, options: ProcessingOptions): boolean {
        // Apply pattern filters
        if (options.includePatterns && options.includePatterns.length > 0) {
            if (!this.matchesPatterns(node.name, options.includePatterns)) {
                return false;
            }
        }
        
        if (options.excludePatterns && options.excludePatterns.length > 0) {
            if (this.matchesPatterns(node.name, options.excludePatterns)) {
                return false;
            }
        }
