...
export class PythonProcessor {
    ...
    private processLineBased(
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (this.isDocstringStart(trimmed)) {
                ...
                if (this.isDocstringEnd(trimmed, docstringQuotes) && trimmed.length > 6) {
                    inDocstring = false;
                    docstringBuffer = [];
                }
                continue;
            ...
            if (trimmed.startsWith('#')) {
                ...
                // Attach comment to next export if includeComments is true
                if (options.includeComments && i < lines.length - 1) {
            ...
            if (trimmed.startsWith('class ')) {
                ...
                const classNode = this.parseClass(trimmed, i + 1, indent, options);
            ...
            if (trimmed.startsWith('def ') || trimmed.startsWith('async def ')) {
                const func = this.parseFunction(trimmed, i + 1, indent, options);
                
                // Check if it's a method inside a class
                if (currentClass && indent > currentIndent) {
                    if (!options.compact && this.shouldIncludeMember(func, options)) {
                        if (!currentClass.members) {
                            currentClass.members = [];
                        }
                        
                        const isPrivate = func.name.startsWith('_');
                        const isSpecial = func.name.startsWith('__') && func.name.endsWith('__');
                        
                        currentClass.members.push({
                            name: func.name,
                            kind: func.name === '__init__' ? 'constructor' : 'method',
            ...
            if (trimmed !== 'pass' && this.isConstantDeclaration(trimmed) && indent === 0) {
            ...
            if (indent === 0 && currentClass && !trimmed.startsWith(' ') && !trimmed.startsWith('\t')) {
                currentClass = null;
            }
        }
    ...
    private parseClass(line: string, lineNumber: number, indent: number, options: ProcessingOptions): ExportNode {
    ...
    private parseFunction(line: string, lineNumber: number, indent: number, options: ProcessingOptions): ExportNode {
    ...
    private parseConstant(line: string, lineNumber: number, options: ProcessingOptions): ExportNode | null {
    ...
    private isConstantDeclaration(line: string): boolean {
        ...
        return /^[A-Z_][A-Z0-9_]*\s*(?::\s*[^=]+)?\s*=/.test(line) ||
    ...
    private shouldIncludeExport(node: ExportNode, options: ProcessingOptions): boolean {
        ...
        if (node.visibility === 'private') {
            ...
            // Include if includePrivate is true, or if it's a special method
            if (!options.includePrivate && !isSpecial) {
                return false;
            }
        }
        
        // Check depth (but respect includePrivate override)
        if (options.depth === 'public' && node.visibility !== 'public' && !options.includePrivate) {
            return false;
        }
    ...
    private getSkipReason(node: ExportNode, options: ProcessingOptions): 'private' | 'pattern' | 'depth' | 'comment' {
