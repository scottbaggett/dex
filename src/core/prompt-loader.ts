...
export class PromptLoader {
    ...
    private loadProjectPrompts(): void {
        ...
        if (existsSync(projectPromptsDir)) {
            try {
                const files = readdirSync(projectPromptsDir).filter(
                    (f) =>
                        f.endsWith(".yml") ||
                        f.endsWith(".yaml") ||
                        f.endsWith(".json"),
                ...
                for (const file of files) {
                    try {
                        ...
                        console.warn(
                            `Failed to load project prompt ${file}:`,
                            error,
                        );
                    }
                }
    ...
    getPrompt(
        ...
        let prompt =
            this.userPrompts.get(id) ||
            this.projectPrompts.get(id) ||
            this.prompts.get(id);
