...
export class DexHelpFormatter extends Help {
     ...
     */
    override formatHelp(cmd: Command, _helper: Help): string {
        const v = cmd.version() || "-";
        const lines: string[] = [];
        ...
        lines.push(
            "  All commands save to .dex/ with pattern: dex.command.context.ext",
        );
        lines.push(
            "  Use -c/--clipboard to copy to clipboard instead of saving to file",
        );
        ...
        lines.push(
            "  dex --help-extended   full option list (original Commander)",
        );
        lines.push("  dex help <command>    command-specific docs");
        lines.push(
            "  dex help-selection    file selection sorting/filtering options\n",
        );

        // Footer
        lines.push(
            chalk.gray("\nDocs: https://github.com/scottbaggett/dex"),
            chalk.gray("Copyright © Scott Baggett"),
        );
