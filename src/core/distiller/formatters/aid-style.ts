import { DistillationResult, ExtractedAPI } from '../../../types';

export class AidStyleFormatter {
  formatDistillation(result: DistillationResult, originalPath: string): string {
    let output = '';
  ...
  private groupFilesByDirectory(apis: ExtractedAPI[]): Map<string, ExtractedAPI[]> {
    ...
    for (const dir of dirs) {
      const files = grouped.get(dir)!.sort((a, b) => a.file.localeCompare(b.file));
  ...
  private formatFile(api: ExtractedAPI): string {
    ...
    // Format exports
    const publicExports = api.exports.filter(e => e.visibility === 'public');
    const privateExports = api.exports.filter(e => e.visibility === 'private');
  ...
  private extractImportsFromExports(api: ExtractedAPI): string[] {
    ...
    for (const exp of api.exports) {
      ...
      const typeMatches = exp.signature.matchAll(/:\s*([A-Z]\w+)(?:<|>|\s|$)/g);
  ...
  private formatInterface(exp: { name: string; signature: string; members?: Array<{ name: string; signature: string; type: 'property' | 'method' }> }): string {
  ...
  private formatClass(exp: { name: string; signature: string; members?: Array<{ name: string; signature: string; type: 'property' | 'method' }> }): string {
    ...
    if (exp.members) {
      // Constructor first
      const constructor = exp.members.find(m => m.name === 'constructor');
      if (constructor) {
