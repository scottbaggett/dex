...
class InternalHelper {
    helper() {}
        ...
        fs.writeFileSync(path.join(testDir, 'utils.py'), `
def public_util():
...
_INTERNAL_FLAG = False
        ...
        fs.writeFileSync(path.join(testDir, 'lib', 'helper.ts'), `
...
export interface HelperConfig {
    ...
    timeout?: number;
        ...
        fs.writeFileSync(path.join(testDir, 'test.spec.ts'), `
describe('test', () => {
    it('should work', () => {
    ...
    function isCombinedResult(result: any): result is { compression: CompressionResult; distillation: DistillationResult } {
    ...
    describe('basic distillation', () => {
        test('should distill all files with default options', async () => {
            ...
            if (isDistillationResult(result)) {
                ...
                expect(tsFile?.exports.some(e => e.name === 'UserService')).toBe(true);
                expect(tsFile?.exports.some(e => e.name === 'createUser')).toBe(true);
                
                // Should include exports from Python files
                const pyFile = result.apis.find(api => api.file.includes('utils.py'));
                expect(pyFile).toBeDefined();
                expect(pyFile?.exports.some(e => e.name === 'public_util')).toBe(true);
                expect(pyFile?.exports.some(e => e.name === 'UtilClass')).toBe(true);
            }
        });
        ...
        test('should respect includePrivate option', async () => {
            ...
            if (isDistillationResult(result)) {
                const pyFile = result.apis.find(api => api.file.includes('utils.py'));
                expect(pyFile?.exports.some(e => e.name === '_private_util')).toBe(true);
                expect(pyFile?.exports.some(e => e.name === '_INTERNAL_FLAG')).toBe(true);
                
                const tsFile = result.apis.find(api => api.file.includes('index.ts'));
                const userService = tsFile?.exports.find(e => e.name === 'UserService');
                expect(userService?.members?.some(m => m.name === 'validateUser')).toBe(true);
            }
        });
        ...
        test('should respect depth option', async () => {
            ...
            if (isDistillationResult(result)) {
                ...
                const userService = tsFile?.exports.find(e => e.name === 'UserService');
                expect(userService?.members?.some(m => m.name === 'getUser')).toBe(true);
                expect(userService?.members?.some(m => m.name === 'logAccess')).toBe(true);
                expect(userService?.members?.some(m => m.name === 'validateUser')).toBe(false);
            }
    ...
    describe('file filtering', () => {
        test('should respect includePatterns', async () => {
            ...
            if (isDistillationResult(result)) {
                expect(result.apis.every(api => api.file.endsWith('.ts'))).toBe(true);
                expect(result.apis.some(api => api.file.endsWith('.py'))).toBe(false);
            }
        });
        ...
        test('should respect excludePatterns', async () => {
            ...
            if (isDistillationResult(result)) {
                expect(result.apis.some(api => api.file.includes('test.spec.ts'))).toBe(false);
                expect(result.apis.some(api => api.file.includes('lib/helper.ts'))).toBe(false);
                expect(result.apis.some(api => api.file.includes('index.ts'))).toBe(true);
            }
        });
        ...
        test('should combine include and exclude patterns', async () => {
            ...
            if (isDistillationResult(result)) {
                expect(result.apis.some(api => api.file.includes('index.ts'))).toBe(true);
                expect(result.apis.some(api => api.file.includes('lib/helper.ts'))).toBe(false);
                expect(result.apis.some(api => api.file.includes('utils.py'))).toBe(false);
            }
        });
    });
    ...
    describe('name filtering', () => {
        test('should respect excludeNames pattern', async () => {
            ...
            if (isDistillationResult(result)) {
                const tsFile = result.apis.find(api => api.file.includes('index.ts'));
                expect(tsFile?.exports.some(e => e.name === 'DEFAULT_USER')).toBe(false);
                expect(tsFile?.exports.some(e => e.name === 'UserService')).toBe(true);
                
                // InternalHelper should not be exported anyway (not exported)
                expect(tsFile?.exports.some(e => e.name === 'InternalHelper')).toBe(false);
            }
        });
    });
    ...
    describe('structure information', () => {
        test('should include structure data', async () => {
            ...
            if (isDistillationResult(result)) {
                ...
                expect(result.structure.languages['typescript']).toBeGreaterThan(0);
    ...
    describe('error handling', () => {
        test('should handle non-existent directory', async () => {
            const distiller = new Distiller({});
            await expect(distiller.distill('/non/existent/path')).rejects.toThrow();
        });
        ...
        test('should handle malformed files gracefully', async () => {
            fs.writeFileSync(path.join(testDir, 'broken.ts'), 'export class { broken syntax');
            ...
            if (isDistillationResult(result)) {
                ...
                const brokenFile = result.apis.find(api => api.file.includes('broken.ts'));
