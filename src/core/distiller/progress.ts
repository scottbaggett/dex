...
export class DistillerProgress {
  ...
  update(filesProcessed: number, cumulativeOriginalBytes: number, cumulativeDistilledBytes: number): void {
  ...
  private updateProgress(): void {
    ...
    const text = chalk.cyan('✨ Distilling ') + 
  ...
  complete(result: { originalTokens: number; distilledTokens: number; fileCount: number }): void {
    ...
    console.log(
      ...
      chalk.green('💰 ~' + Math.round((result.originalTokens - result.distilledTokens) / 1000) + 'k tokens saved ') +
      chalk.gray(`(~${Math.round(result.distilledTokens / 1000)}k remaining)`)
    );
  }
