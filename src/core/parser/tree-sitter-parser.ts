...
export class TreeSitterParser extends BaseParser {
        ...
        switch (node.type) {
            ...
            case "function_declaration":
                if (this.isExported(node)) {
                    this.extractTypeScriptFunction(
                        node,
                        content,
                        lines,
                        exports,
            ...
            case "interface_declaration":
                if (this.isExported(node)) {
                    this.extractTypeScriptInterface(
                        node,
                        content,
                        lines,
                        exports,
            ...
            case "type_alias_declaration":
                if (this.isExported(node)) {
                    this.extractTypeScriptTypeAlias(
                        node,
                        content,
                        lines,
                        exports,
    ...
    private extractTypeScriptFunction(
        ...
        exports.push({
            ...
            signature,
            visibility:
                isExported || this.isExported(node) ? "public" : "private",
    ...
    private isRustPublic(node: any): boolean {
        ...
        for (let i = 0; i < node.childCount; i++) {
            ...
            if (
                ...
                this.getNodeText(child, "").includes("pub")
            ) {
                return true;
            }
        }
        ...
        this.walkAST(body, (child: any) => {
            if (child.type === "function_definition") {
                ...
                if (nameNode) {
                    members.push({
                        name: this.getNodeText(nameNode, content),
                        signature: this.getPythonFunctionSignature(
                            child,
                            content,
                        ),
                        type: "method",
    ...
    private getClassSignature(node: any, content: string): string {
        ...
        return braceIndex > -1
            ? fullText.substring(0, braceIndex).trim()
            : fullText;
    }
    ...
    private getPythonFunctionSignature(node: any, content: string): string {
        ...
        return colonIndex > -1
            ? fullText.substring(0, colonIndex).trim()
            : fullText;
    }
