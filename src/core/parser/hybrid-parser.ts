...
export class HybridParser extends BaseParser {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      console.warn('Tree-sitter parser initialization failed, using regex fallback only');
    }
  ...
  extract(parsedFile: ParsedFile, depth: DistillDepth): ExtractedAPI {
    ...
    if (ast && this.treeSitterParser.isLanguageSupported(language)) {
      try {
        ...
        console.warn(`Tree-sitter extraction failed for ${language}, falling back to regex:`, error);
  ...
  isLanguageSupported(language: string): boolean {
    return this.treeSitterParser.isLanguageSupported(language) || this.regexParser.isLanguageSupported(language);
  }
