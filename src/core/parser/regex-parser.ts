...
export class RegexParser extends BaseParser {
    ...
    if (language === 'typescript' || language === 'javascript') {
      ...
      while ((match = importRegex.exec(content)) !== null) {
        if (match[1]) {
          ...
          const modules = match[2].split(',').map(m => m.trim().split(' as ')[0]);
  ...
  private extractTypeScriptExports(content: string): EnhancedExport[] {
    ...
    while ((match = interfaceRegex.exec(content)) !== null) {
      ...
      const members = this.extractInterfaceMembers(content, match.index + match[0].length);
    ...
    const classRegex = /^(?:export\s+)?(?:abstract\s+)?class\s+(\w+)(?:<[^>]+>)?(?:\s+extends\s+[^{]+)?(?:\s+implements\s+[^{]+)?\s*\{/gm;
    while ((match = classRegex.exec(content)) !== null) {
      const name = match[1];
      const startLine = content.substring(0, match.index).split('\n').length;
      const members = this.extractClassMembers(content, match.index + match[0].length);
    ...
    const functionRegex = /^(?:export\s+)?(?:async\s+)?function\s+(\w+)\s*(<[^>]+>)?\s*\([^)]*\)(?:\s*:\s*[^{]+)?/gm;
    ...
    const arrowFunctionRegex = /^(?:export\s+)?const\s+(\w+)\s*(?::\s*[^=]+)?\s*=\s*(?:async\s+)?\([^)]*\)\s*(?::\s*[^=]+)?\s*=>/gm;
  ...
  private extractInterfaceMembers(content: string, startPos: number): Array<{name: string; signature: string; type: 'property' | 'method'}> {
    ...
    while (braceCount > 0 && currentPos < content.length) {
      ...
      if (char === '{') braceCount++;
        ...
        const memberText = content.substring(memberStart, currentPos).trim();
        if (memberText && !memberText.startsWith('//')) {
          const propertyMatch = memberText.match(/^\s*(?:readonly\s+)?(\w+)(?:\?)?\s*:\s*(.+)$/);
          const methodMatch = memberText.match(/^\s*(\w+)\s*\([^)]*\)(?:\s*:\s*.+)?$/);
  ...
  private extractClassMembers(content: string, startPos: number): Array<{name: string; signature: string; type: 'property' | 'method'}> {
    ...
    // Extract properties
    const propertyRegex = /^\s*(?:public\s+|private\s+|protected\s+)?(?:static\s+)?(?:readonly\s+)?(\w+)(?:\?)?\s*(?::\s*[^;]+)?(?:\s*=\s*[^;]+)?;/gm;
    let match;
    ...
    // Extract methods
    const methodRegex = /^\s*(?:public\s+|private\s+|protected\s+)?(?:static\s+)?(?:async\s+)?(\w+)\s*\([^)]*\)(?:\s*:\s*[^{]+)?/gm;
  ...
  private extractPythonExports(content: string): EnhancedExport[] {
    ...
    while ((match = classRegex.exec(content)) !== null) {
      const name = match[1];
      const startLine = content.substring(0, match.index).split('\n').length;
      const members = this.extractPythonClassMembers(content, match.index + match[0].length, startLine);
  ...
  private extractPythonClassMembers(content: string, startPos: number, classStartLine: number): Array<{name: string; signature: string; type: 'property' | 'method'}> {
    const members: Array<{name: string; signature: string; type: 'property' | 'method'}> = [];
    const lines = content.split('\n');
    ...
    for (let i = classStartLine; i < lines.length; i++) {
      ...
      // Look for methods
      const methodMatch = line.match(/^\s+def\s+(\w+)\s*\([^)]*\)(?:\s*->\s*[^:]+)?:/);
      if (methodMatch) {
        members.push({
          name: methodMatch[1],
