...
export class StructuredFormatter implements Formatter {
    ...
    private formatFile(api: ExtractedAPI, options: FormatterOptions): string {
        ...
        if (options.groupByType !== false) {
            ...
            const order = ['interface', 'class', 'function', 'type', 'const', 'enum'];
    ...
    private formatClass(exp: any, options: FormatterOptions): string {
        ...
        const implementsMatch = exp.signature.match(/implements\s+(.+?)(?:\s*{|$)/);
        ...
        if (exp.members && !options.compact) {
            // Constructor first
            const constructor = exp.members.find((m: any) => m.name === 'constructor');
            if (constructor) {
                output += `    ${constructor.signature}\n`;
            }
            
            // Properties
            const properties = exp.members.filter((m: any) => m.type === 'property');
            for (const prop of properties) {
                output += `    ${prop.signature};\n`;
            }
            
            // Methods
            const methods = exp.members.filter((m: any) => 
