...
export class FileSelector {
     ...
     */
    private async enhanceGitChanges(
        changes: GitChange[],
    ): Promise<EnhancedGitChange[]> {
        try {
            // Get git status info
            ...
            return changes.map((change) => {
                ...
                enhanced.isUnstaged =
                    modifiedFiles.has(change.file) && !enhanced.isStaged;
    ...
    async selectFiles(
        ...
        // Apply sorting
        const sortedChanges = this.sortChanges(
            filteredChanges,
            options.sortBy,
            options.sortOrder,
        );
        ...
        if (options.sortBy) {
            console.log(
                chalk.blue(
                    `Sorted by ${options.sortBy} (${options.sortOrder || "asc"})`,
                ),
            );
        }
        ...
        if (!process.stdin.isTTY || !process.stdin.setRawMode) {
            ...
            console.log(
                chalk.yellow(
                    "Falling back to selecting all files in non-interactive mode.",
                ),
            );
            return {
                files: sortedChanges,
    ...
    showTTYError() {
        ...
        console.log(
            chalk.yellow(
                "Interactive mode is not available in this environment.",
            ),
        );
    ...
    static getOptionsHelp(): string {
        ...
        help.push(
            "    • status             Git status (added/modified/deleted)",
        );
        help.push(
            "  --sort-order <order>   Sort direction: asc (default) or desc",
        );
        ...
        help.push(
            "    • unstaged           Unstaged changes " + chalk.yellow("●"),
        );
        help.push(
            "    • untracked          Untracked files " + chalk.gray("?"),
        );
        help.push(
            "    • modified           Modified files " + chalk.yellow("M"),
        );
