...
export function createDistillCommand(): Command {
  ...
  command
    ...
    .option('--exclude <patterns...>', 'Exclude file patterns', collectPatterns, [])
    ...
    .option('--ai-action <action>', 'Generate AI prompt (audit, refactor, document, analyze)')
...
async function distillCommand(targetPath: string, options: any): Promise<void> {
  ...
  try {
    ...
    if (options.aiAction) {
      output = generateAIPrompt(options.aiAction, formatted) + '\n\n' + output;
    }
