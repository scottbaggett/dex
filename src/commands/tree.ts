...
import { DistillerOptions, ExtractedAPI, ProjectStructure } from '../types';
...
async function treeCommand(targetPath: string, options: any): Promise<void> {
  try {
    ...
    const cliExcludes = Array.isArray(options.exclude) ? options.exclude : [];
    ...
    if (!('apis' in result)) {
      console.error(chalk.red('No API information extracted. Try a different depth level.'));
      process.exit(1);
    }
    ...
    const tree = generateTree(result.apis, result.structure, options, resolvedPath, forTerminal);
...
function generateTree(
  ...
  const allExports = apis.flatMap((api) => api.exports.map((exp) => ({ ...exp, file: api.file })));

  const filteredExports = allExports.filter((exp) => {
    if (!includePrivate && (exp.visibility === 'private' || exp.name.startsWith('_'))) {
      return false;
    }
    return true;
  });
  ...
  if (format === 'outline') {
    return generateOutline(filteredExports, basePath, showTypes, showParams, forTerminal);
  }
...
function generateTreeFormat(
  ...
  if (groupBy === 'file') {
    return generateTreeByFile(exports, basePath, showTypes, showParams, forTerminal);
...
function generateTreeByFile(
  ...
  lines.push(forTerminal ? chalk.bold.cyan(`📁 ${basename(basePath)}`) : basename(basePath));

  renderDirectoryTree(tree, exportsByFile, lines, '', true, showTypes, showParams, forTerminal);
...
function renderDirectoryTree(
  ...
  const directories = Array.from(node.children.entries()).sort(([a], [b]) => a.localeCompare(b));
  ...
  for (const filePath of files) {
    ...
    lines.push(`${prefix}${filePrefix}${forTerminal ? chalk.blue(fileName) : fileName}`);

    // Render exports for this file
    const fileExports = exportsByFile.get(filePath) || [];
    const sortedExports = fileExports.sort((a, b) => {
      if (a.type !== b.type) {
        const typeOrder = { class: 0, function: 1, interface: 2, type: 3, const: 4, enum: 5 };
        return (
    ...
    for (let j = 0; j < sortedExports.length; j++) {
      ...
      if (showTypes && exp.signature) {
        exportLine += forTerminal ? chalk.gray(` ${exp.signature}`) : ` ${exp.signature}`;
      }
      ...
      if (exp.docstring) {
        const descIndent = baseIndent + (isLastExport ? '    ' : '│   ');
        const desc = exp.docstring.slice(0, 80) + (exp.docstring.length > 80 ? '...' : '');
        lines.push(`${descIndent}${forTerminal ? chalk.dim(desc) : desc}`);
      }
    }
...
function generateTreeByType(
  ...
  const typeOrder = ['class', 'interface', 'type', 'function', 'const', 'enum'];
  ...
  for (let i = 0; i < sortedTypes.length; i++) {
    ...
    const sortedExports = typeExports.sort((a, b) => a.name.localeCompare(b.name));
...
function generateFlatTree(
  ...
  for (let i = 0; i < sortedExports.length; i++) {
    ...
    if (showTypes && exp.signature) {
      line += forTerminal ? chalk.gray(` ${exp.signature}`) : ` ${exp.signature}`;
    }
    ...
    if (exp.docstring) {
      const desc = exp.docstring.slice(0, 100) + (exp.docstring.length > 100 ? '...' : '');
      const indent = isLast ? '    ' : '│   ';
...
function getTypeColor(type: string, forTerminal: boolean = false): (text: string) => string {
  if (!forTerminal) {
...
async function handleOutput(
  ...
  if (options.clipboard) {
    ...
    console.log(chalk.cyan('💾 API tree written to: ') + chalk.green(options.output));
    ...
    console.log(chalk.cyan('💾 API tree saved to: ') + chalk.green(relativePath));
